...
async function syncRouteQuery(teacherId: string): Promise<void> {
  if (switchingWorkShell.value) {
    return
  }
...
<template>
  <div class="portfolio-scope-header">
    ...
    <div class="portfolio-scope-header__field portfolio-scope-header__field--grow">
      <span id="portfolio-current-teacher-label" class="portfolio-scope-header__label">当前教师</span>
      <template v-if="canPickTeachers">
        <template v-if="!selectedTeacherId">
          ...
          <UiSelect
            ...
            :disabled="switchingWorkShell"
            placeholder="或快速搜索教师"
            aria-labelledby="portfolio-current-teacher-label"
            :filter-option="false"
