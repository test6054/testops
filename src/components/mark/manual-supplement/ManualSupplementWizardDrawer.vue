  ...
  >
    ...
    <div v-if="currentStep === 0" class="manual-supplement-wizard__panel">
      ...
      <template v-else>
        ...
        <p v-if="paperPageStatusLoadFailed" class="manual-supplement-wizard__warn muted">
          卷面页状态加载失败；缺页补扫/污损替换须先成功确认当前卷面页状态，不能继续。
        </p>
        <p v-else-if="scenarioBlockReason" class="manual-supplement-wizard__warn muted">
          {{ scenarioBlockReason }}
        </p>
      </template>
    </div>
...
const classScopeWarning = computed(() => {
  if (examDetailLoadFailed.value) {
    return '考试详情加载失败，班级范围暂不可用'
  }
  return declaredClassIds.value.length === 0 ? '请先在考生名册维护考试班级范围' : ''
})

const submitDisabled = computed(
  () =>
    examDetailLoadFailed.value
