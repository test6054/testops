  ...
  >
    ...
    >
      ...
      <section class="exam-edit-drawer__section">
        ...
        <div
          v-if="!allowDailyScoreComposition"
          class="exam-edit-drawer__status-row"
          aria-label="成绩构成"
        >
          <div class="exam-edit-drawer__status-row-head">
            <span class="exam-edit-drawer__status-row-label">成绩构成</span>
            <UiTag tone="blue" size="sm">本场固定</UiTag>
          </div>
          <p class="exam-edit-drawer__status-row-value">仅卷面成绩</p>
          <p class="exam-edit-drawer__hint">
            {{ examKindMessageLabel }}按卷面成绩计分，不参与平时成绩合成，本项不可编辑。
          </p>
        </div>

        <UiFormItem v-else label="成绩构成" name="scoreCompositionMode">
          <div
    ...
    <template #footer>
      <template v-if="detailLoadFailed === true">
        <UiButton size="sm" variant="outline" @click="emit('update:open', false)">
          关闭
        </UiButton>
      </template>
      <template v-else-if="detailLoading !== true">
        <UiButton size="sm" variant="outline" :disabled="saving === true" @click="emit('update:open', false)">
          取消
        </UiButton>
        <UiButton
...
import { ExamKindCode, ExamKindDescription } from '@/types/enums/exam-kind-enum'
...
/**
 * 本场考试性质；仅正考可配平时成绩。
 * 补考/缓考/重考/重修的计分策略（补考封顶60 / 仅卷面实际分）在成绩确认时强制平时分为 0，
 * 若这些考试仍带平时满分，合成满分会把平时满分计入分母，稀释学生端与质量评价的「实得分/满分」。
 */
const examKind = ref<ExamKindCode | null>(null)
const allowDailyScoreComposition = computed(
  () => examKind.value === null || examKind.value === ExamKindCode.REGULAR,
)
const examKindMessageLabel = computed(() =>
  examKind.value === null
    ? '本场考试'
    : strictEnumLabel(ExamKindDescription, examKind.value, '考试性质'),
)
...
async function loadDetail(examId: string): Promise<void> {
  ...
  try {
    ...
    examKind.value = detail.examKind
    examForm.examWindow
...
function buildUpdateRequest(): ExamUpdateRequest | null {
  ...
  return {
    ...
    // 非正考恒不带平时满分，与 BE examKind 不变式同源，避免残留态被提交
    dailyScoreFull:
      allowDailyScoreComposition.value && examForm.scoreCompositionMode === 'EXAM_WITH_DAILY'
...
async function handleSave(): Promise<void> {
  ...
  if (detailLoading.value === true || detailLoadFailed.value === true) {
