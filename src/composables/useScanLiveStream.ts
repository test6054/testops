/**
 * 扫描实时看板 composable：管理 SSE 订阅、断线补差、滚动缓冲区。
 *
 * 用法示例：
 *   const { events, ready, error, isStreaming, refresh, start, stop }
 *     = useScanLiveStream({ filter: () => ({ examId: selectedExamId.value }) })
 *   onMounted(start)
...
export function useScanLiveStream(
  ...
  async function start(): Promise<void> {
    ...
    controller = subscribeScanLive(filter, {
      onReady: () => {
        ready.value = true
        // 收到 ready 后做一次断线补差（拿历史与 SSE 之间的窗口期事件）
