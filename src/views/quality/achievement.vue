...
const triggerButtons: Array<{
  key: AchievementComputeKindCode
  label: string
  handler: () => Promise<AchievementComputeResult>
}> = [
  {
    key: AchievementComputeKindCode.COURSE_GOAL,
    label: '课程目标',
    handler: () => {
  ...
  {
    key: AchievementComputeKindCode.REQUIREMENT,
    label: '毕业要求 / 观测点',
    handler: () =>
  ...
  {
    key: AchievementComputeKindCode.TRAINING_OBJECTIVE,
    label: '培养目标',
    handler: () => {
  ...
  {
    key: AchievementComputeKindCode.PROGRAM,
    label: '专业汇总',
    handler: () =>
  ...
  {
    key: AchievementComputeKindCode.CIVIC_GOAL_AGGREGATE,
    label: '课程思政',
    handler: () =>
  ...
  {
    key: AchievementComputeKindCode.COMPLEX_ENGINEERING,
    label: '复杂工程',
    handler: () =>
...
async function handleTrigger(key: AchievementComputeKindCode, handler: () => Promise<AchievementComputeResult>) {
  try {
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'sample'">
              <div class="achievement__sample">
                <span>{{ record.sampleValid }} / {{ record.sampleTotal }}</span>
                <span
                  v-if="record.sampleTotal > record.sampleValid"
                  class="dp-text-muted-xs"
                >含无效 {{ record.sampleTotal - record.sampleValid }}</span>
              </div>
            </template>
      ...
      <section class="achievement__drawer-section">
        ...
        <div v-else class="achievement__trigger-chain">
          ...
          >
            ...
            <UiButton
              :variant="step.key === AchievementComputeKindCode.COURSE_GOAL ? 'primary' : 'outline'"
              size="sm"
            ...
            >
              {{ step.key === AchievementComputeKindCode.COURSE_GOAL ? '计算课程目标' : `计算${step.label}` }}
            </UiButton>
          </div>
        </div>
      </section>
