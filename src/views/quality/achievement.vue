...
const achievementResultItems = computed<TaskResultItem[]>(() => {
  ...
  return list.value
    .filter(
      (r) =>
        r.auditStatus === 'RETURNED' || r.achievementStatus === 'NOT_ACHIEVED' || isResultStale(r),
    )
    .slice(0, 5)
    .map((r) => ({
      id: r.id,
      ...
      statusLabel:
        r.auditStatus === 'RETURNED' ? '已驳回' : isResultStale(r) ? '结果过期' : '未达成',
