...
/** 首屏只展示最紧急待办，完整列表仍可在展开区查看。 */
const urgentTodos = computed(() => todos.value.slice(0, 5))
    ...
    <div v-else class="teacher-home__layout">
      ...
      <div class="teacher-home__grid">
        ...
        <WorkbenchSurfaceCard class="teacher-home__todos">
          <template #head>
            <div class="teacher-home__panel-head">
              <h3 class="teacher-home__panel-title">最紧急待办</h3>
          ...
          <UiSpin :spinning="todoLoading || workbenchSummaryLoading">
            <ul v-if="urgentTodos.length" class="teacher-home__todo-list">
              <li
            ...
            <p
              v-if="todos.length > urgentTodos.length"
              class="teacher-home__meta"
            >
              首屏展示前 {{ urgentTodos.length }} 项；其余待办请刷新后按截止排序处理。
            </p>
            <UiEmpty
              size="sm"
