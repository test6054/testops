...
const KeyTeacherSignalMetrics = computed<SignalMetric[]>(() => {
...
const KeyTeacherWorkbenchSubtitle = computed(() => {
  if (loadError.value) {
    return '加载失败'
  }
...
function onKeyTeacherSignalClick(_key: string) {
  void loadPage()
}
...
async function confirmExportApply() {
  ...
  if (!tenantWide && !departmentId) {
    showFormValidationMessage('当前账号缺少院系范围，无法申请导出骨干教师台账')
  ...
  try {
    await portfolioSecurityApi.applyExport({
      exportType: PortfolioExportTypeCode.KEY_TEACHER_ROSTER,
      businessRef: {
        keyTeacherRegistryType: activeType.value,
        ...(tenantWide ? {} : { departmentId }),
      },
      exportPurpose,
    })
    exportApplyModal.open = false
    void message.success('已提交骨干教师台账导出审批')
    void router.push({ name: 'PortfolioExportApprovalMine' })
  } catch (error) {
    showUserError(error, '提交骨干教师台账导出审批失败')
...
<template>
  <StageWorkbenchShell>
    <template #context>
      <ContextBar layout="workbench" show-title :title="pageTitle" :subtitle="KeyTeacherWorkbenchSubtitle" />
    </template>
    <template v-if="KeyTeacherSignalMetrics.length > 0" #signal>
      <SignalBand
        layout="spotlight"
        variant="inline"
        compact
        :metrics="KeyTeacherSignalMetrics"
        @metric-click="onKeyTeacherSignalClick"
      />
    </template>
    <UiAlertStrip
