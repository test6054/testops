  ...
  <GiPageLayout>
    <div class="experience-page">
      <PageHeader title="批改经验库">
        <template #tags>
          <UiTag v-if="signatures.length > 0" tone="blue" size="md">签名 {{ signatures.length }}</UiTag>
          <UiTag v-if="experiences.length > 0" tone="green" size="md">经验案例 {{ experiences.length }}</UiTag>
        </template>
        <template #actions>
          <a-select
      ...
      <a-tabs v-else v-model:active-key="activeTab">
            ...
            >
              <template #bodyCell="{ column, record }">
                <template v-if="column.key === 'questionType'">
                  {{ record.questionType ? QUESTION_TYPE_LABEL[record.questionType as QuestionTypeCode] ?? record.questionType : '-' }}
                </template>
              ...
              <template #bodyCell="{ column, record }">
                ...
                <template v-else-if="column.key === 'analysisStatus'">
                  <UiTag :tone="aiStatusTone(record.analysisStatus)" size="sm">
                    {{ AI_ANALYSIS_STATUS_LABEL[record.analysisStatus as AiAnalysisStatusCode] ?? record.analysisStatus }}
                  </UiTag>
                </template>
                <template v-else-if="column.key === 'caseStatus'">
                  <UiTag :tone="caseStatusTone(record.caseStatus)" size="sm">
                    {{ EXPERIENCE_CASE_STATUS_LABEL[record.caseStatus as ExperienceCaseStatusCode] ?? record.caseStatus }}
                  </UiTag>
                </template>
                ...
                <template v-else-if="column.key === 'actions'">
                  <UiButton size="xs" variant="outline" @click="openExperienceDrawer(record)">详情</UiButton>
                </template>
              </template>
            </a-table>
          </UiCard>
        ...
        <a-tab-pane key="cluster">
          ...
          <UiCard class="info-card">
            <template #title>
              <span>答案聚类</span>
              <UiBadge v-if="latestCluster" :tone="aiStatusTone(latestCluster.analysisStatus ?? 'PENDING')">
            ...
            <a-empty v-if="!latestCluster" description="尚无聚类结果，请先指定题目模板ID 并点击「AI 聚类」" />
          ...
          <template #renderItem="{ item }">
            <a-list-item>
              <a-space size="small">
                ...
                <UiTag tone="cyan" size="sm">{{ item.questionType ? QUESTION_TYPE_LABEL[item.questionType as QuestionTypeCode] : '-' }}</UiTag>
    ...
    >
      <a-descriptions v-if="detailExperience" :column="1" bordered size="small">
        ...
        <a-descriptions-item label="来源考试">{{ detailExperience.sourceExamId }}</a-descriptions-item>
        <a-descriptions-item label="题目模板">{{ detailExperience.questionTemplateId }}</a-descriptions-item>
        <a-descriptions-item label="状态">
          <UiTag :tone="caseStatusTone(detailExperience.caseStatus)" size="sm">
            {{ EXPERIENCE_CASE_STATUS_LABEL[detailExperience.caseStatus as ExperienceCaseStatusCode] ?? detailExperience.caseStatus }}
          </UiTag>
        </a-descriptions-item>
        <a-descriptions-item label="AI 状态">
          <UiTag :tone="aiStatusTone(detailExperience.analysisStatus)" size="sm">
            {{ AI_ANALYSIS_STATUS_LABEL[detailExperience.analysisStatus as AiAnalysisStatusCode] ?? detailExperience.analysisStatus }}
          </UiTag>
        </a-descriptions-item>
        <a-descriptions-item label="AI Trace">{{ detailExperience.aiTraceId ?? '-' }}</a-descriptions-item>
        <a-descriptions-item label="耗时">
        ...
        <a-descriptions-item label="适用边界">{{ detailExperience.applicableScope ?? '-' }}</a-descriptions-item>
