  ...
  <StageWorkbenchShell>
    ...
    <template v-else>
      ...
      <UiSectionTabs
        ...
        compact
        class="scan-batch-workbench__status-tabs"
...
const filterForm = reactive({
  keyword: '',
  scannerDeviceId: undefined as string | undefined,
  scanWindow: undefined as [string, string] | undefined,
})
...
const statusTabItems = computed(() => [
  ...
  { key: 'IN_PROGRESS', label: '进行中', count: summary.value?.inProgressCount },
  { key: 'BLOCKED', label: '阻断', count: summary.value?.blockedCount },
...
const filterFields = computed(() => [
  {
    ...
    inputPrefixIcon: 'search' as const,
  },
  {
    key: 'scannerDeviceId',
    label: '扫描设备',
...
function syncFilterForm(next: Record<string, unknown>): void {
  ...
  filterForm.scannerDeviceId = next.scannerDeviceId as string | undefined
  filterForm.scanWindow = next.scanWindow as [string, string] | undefined
}
...
.scan-batch-workbench__status-tabs {
  margin-bottom: 12px;
}
