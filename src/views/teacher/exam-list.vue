        ...
        >
          <template #bodyCell="{ column, record }">
            <template v-if="column.key === 'examName'">
              <div class="exam-list-page__exam-name-cell">
                <div class="exam-list-page__exam-name-row">
                  ...
                  <template v-for="phaseTag in [examWindowPhaseTag(record)]" :key="phaseTag?.label">
            ...
            <template v-else-if="column.key === 'progress'">
              ...
              <span v-else-if="record.questionCount <= 0" class="dp-text-muted">无题目</span>
              <span v-else class="dp-text-muted">—</span>
            </template>
...
import {
  requireExamEntryWorkspaceRouteName,
...
/**
 * 是否本租户企业管理员全量考试视角。
 * 与后端 ExamMarkPermissionService.hasFullTenantReadView() 对齐：仅 edu-user TenantAdmin；
 * 不得使用 isTenantAdmin（其对 SUPER_ADMIN 恒 true）。
 */
const isAdminView = computed(() => userStore.isEnterpriseTenantAdmin === true)
...
/**
 * 列表进入工作台：只认行上 workspaceRouteName / enterActionLabel 合同。
 * 优先原因、原因筛选覆写、非优先进度默认入口均由后端唯一决议，禁止 FE 启发。
 */
function goSmartExamEntry(exam: ExamWorkbenchSummaryResponse): void {
  const examId = exam.examId
  try {
    const reasonFilter = priorityReasonFilter.value
    if (reasonFilter && !exam.priorityReasonCodes?.includes(reasonFilter)) {
      throw new Error(
        `考试 ${exam.examId} 未包含深链原因 ${reasonFilter}，与服务端过滤合同不一致`,
      )
    }
    const routeName = requireExamEntryWorkspaceRouteName(exam)
...
function resolveExamEnterActionLabel(exam: ExamWorkbenchSummaryResponse): string {
  ...
  if (!label) {
    throw new Error(`考试 ${exam.examId} 缺少 enterActionLabel 合同字段`)
  }
  return label
}
...
function confirmClose(exam: ExamWorkbenchSummaryResponse): void {
  if (canManageOwnerExamLifecycle(exam) !== true || examActionLoading.value === true || batchLifecycle.running === true) {
    return
  }
  void (async () => {
    try {
      const readiness = await getExamCloseReadiness(exam.examId)
      if (readiness.closeExamReady !== true) {
        const firstBlockingItem = readiness.blockingItems[0]
        void confirmAsync({
          title: '尚不能关考',
          content: buildCloseExamBlockedContent(readiness),
          okText: firstBlockingItem?.actionLabel ?? '知道了',
          cancelText: '知道了',
          type: 'warning',
          onOk: async () => {
            if (firstBlockingItem?.targetRouteName) {
              await router.push({
                name: firstBlockingItem.targetRouteName,
      ...
      void confirmAsync({
        ...
        content: buildCloseExamReadyContent(readiness),
        okText: '关闭考试',
        cancelText: '取消',
        type: 'warning',
        onOk: async () => {
