...
export function buildFourPropertyDimensionViews(
  ...
  return ALL_ARCHIVE_FOUR_PROPERTY_DIMENSION_CODES.map((key) => ({
    key,
    label: strictEnumLabel(ArchiveFourPropertyDimensionDescription, key, '四性检测维度'),
    passed: resolveDimensionPassed(check, key),
    description: messages.get(key) ?? (resolveDimensionPassed(check, key) ? '检测通过' : '未通过'),
  }))
}

export function countFourPropertyPassed(views: ArchiveFourPropertyDimensionView[]): {
  passed: number
  total: number
} {
...
export function resolveSecurityDiagnosticMessage(
  ...
  const message = parseDiagnosticMessages(check?.diagnostic).get(
...
function parseDiagnosticMessages(
  diagnostic?: string,
): Map<ArchiveFourPropertyDimensionCode, string> {
...
function resolveDiagnosticDimension(value: unknown): ArchiveFourPropertyDimensionCode | undefined {
  ...
  if (value === ArchiveFourPropertyDimensionCode.USABILITY)
    return ArchiveFourPropertyDimensionCode.USABILITY
  if (value === ArchiveFourPropertyDimensionCode.SECURITY)
    return ArchiveFourPropertyDimensionCode.SECURITY
  return undefined
}
