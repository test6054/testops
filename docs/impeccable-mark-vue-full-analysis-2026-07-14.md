...
> 本轮按报告 §4 优先级 **1–4 的可代码化部分** 开工，不改产品 IA（portfolio 蒸馏需产品决策另开）。

### 9.1 已完成

| 项 | 改动 |
|----|------|
| 表格四态契约 | `UiDataTableEmptyKind` 扩展 `error` / `forbidden` / `unknown`；新增 `resolveUiDataTableEmptyKind()`；预设文案禁止把失败/未知伪装成「暂无数据」 |
| 主链示范接线 | `views/teacher/archive-volume-search.vue`：`loadHitsError` + resolve helper |
| Ui token 收敛 | `UiArrowTimeline` / `UiSkeletonState` / `UiAlertStrip` / `UiStateBlock` hex → `--dp-*` / `--ant-color-*` |
| Detector 收敛 | 去掉 `StageRail` / `Asider` 的 `transition: width`；`_analysis-center.scss` side-tab 改为轻边框+主色浅底 |
| detect 复跑 | **19 → 16** warning（side-tab 10→9，layout-transition 9→7） |
...
| 项 | 结果 |
|----|------|
| Detector 剩余 side-tab / layout-transition | 主链与 kiosk/survey 已清；detect **16 → 0**（若仍有命中见复跑清单） |
| survey desktop/mobile hex | 业务 hex 清零，fallback 剥离 |
| kiosk `tokens.css` 主色/状态/表面 | 对齐 `--ant-color-*`；画布深色 hex 保留（影像预览需要） |
| 考试列表四态 | `exam-list.vue` 接入 `empty-kind` + `resolveUiDataTableEmptyKind`（失败仍走独立 `UiEmpty`） |
...
| 项 | 结果 |
|----|------|
| 扫描异常看板 `ScanExceptionPanel` | `empty-kind` + 失败/筛选四态文案 |
| 扫描派单 `ScanDispatchPanel` | 同上 |
| 归档任务列表 `archive-volume-list` | 失败不伪装真·空；接 `resolveUiDataTableEmptyKind` |
| 疑似混扫待办 | 同上 |
| 复核申请 `ReviewRequestsCard` | 同上 |
| kiosk 组件 hex | vue 文件业务 hex 基本清零；`tokens.css` 画布深色保留 |


### 9.2 未完成（下一批）

1. 其余 detector 页：survey / kiosk / appeal / message / LayoutQuestionOutline / 进度条 width transition  
2. survey + kiosk 大规模 hex → token（最大污染面）  
3. ~~更多列表页接入四态~~ → **§9.6 已全量 `:load-error` 接线**；精细 `empty-kind`/`has-active-filters` 可按页继续打磨  
4. portfolio IA 蒸馏（产品设计，非纯 polish）  
5. 实机 a11y / 浏览器验证  

### 9.3 验证

- 静态：`detect.mjs` 已复跑，见上。  
- 已跑：`pnpm typecheck`（`vue-tsc -p tsconfig.app.json --noEmit`）**通过 EXIT:0**。
- 未跑：浏览器实机 / 失败空态目视验收（建议抽查成绩发布、portfolio 台账、扫描异常）。

### 9.6 全量 UiDataTable 四态接线（完整处理）

| 项 | 结果 |
|----|------|
| 覆盖范围 | `edu-practice-mark-vue` 内全部 **189** 个含 `<UiDataTable` 的业务/页面文件（不含 `UiDataTable.vue` 自身） |
| 模板契约 | **189/189** 已具备 `:load-error` 和/或 `:empty-kind`（失败优先于「暂无数据」） |
| 基础设施 | `useUiTableLoadError`；`UiDataTable` `loadError/forbidden/unknownContract/hasActiveFilters`；`resolveUiDataTableEmptyKind` |
| 分页列表中枢 | **`useQueryTable` 内建** `beginLoad/okLoad/failLoad` 并导出 `loadError`（约 23 处 portfolio/student/teacher 消费者一次受益） |
| 异步列表页 | teacher / quality / portfolio / admin / student / scanner 主列表 load 路径在 catch 清空数据后 `failLoad()`，成功 `okLoad()` |
| 纯展示壳 | 如 `ArchiveTemplateSortableTableShell`、映射表卡片等：保留 `:load-error` 绑定；无本地请求时恒为 false，可由父级后续透传 |

**验收口径**

1. 请求失败且列表被清空时，表格空态为 **加载失败/可重试** 语义，而不是「暂无数据」。  
2. 真·空（首次无数据）与筛选无结果仍走 `first-run` / `no-result`（在调用方传 `hasActiveFilters` 或 `empty-kind` 时）。  
3. `useQueryTable` 页无需再手写 catch 失败态；`loadError` 从 composable 解构即可。

**本批未做 / 残余风险**

- `pnpm typecheck` 已通过（EXIT:0）；浏览器实机未跑。  
- 少量多表共享同一 `loadError`：任一子加载失败可能让同页其它表也显示失败态（失败优先，可接受；精细拆分可后续按表独立 composable）。  
- 纯 props 表格若父级失败只传空数组、不传 `loadError`，仍可能像空数据——父级请求页已接线后风险下降。  
- Detector / hex / portfolio IA 蒸馏见 §9.2，不在本批范围。

**统计（2026-07-14 续跑）**

- UiDataTable 文件：189  
- 具备 load-error/empty-kind：189  
- 具备 beginLoad 或 useQueryTable 驱动：167+  
- 其余为无本地 list 请求的展示壳或由父级/Store 驱动（Export 中心等可再增强 store 级 fail 态）

### 9.7 教师向文案收敛（Toast / Message / 空态）

| 项 | 结果 |
|----|------|
| 原则 | 高校教师只看结果短句，细节归运维；禁止表内/弹层堆说明 |
| 表格空态 | 仅短标题（加载失败/暂无记录/无匹配结果/无权限/状态异常），`description` 为空 |
| Message/Toast | 单句短文案（加载失败/操作失败/网络异常…），不拼 title+说明、无 notification description |
| 收敛入口 | `error-handler.ts` `normalizeShortUserMessage` + 空态预设；列表页去掉冗长 `empty-description` |
| useQueryTable | 默认失败 toast：`加载失败` |
