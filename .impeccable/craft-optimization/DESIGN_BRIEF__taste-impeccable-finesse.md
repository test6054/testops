...
| 04 正评沉浸给分 | **已确认** |
| 05 发布预检 | **已确认** |
| 06b 表格与多 Tab | **待确认（看板已补 #scene-06b）** · 推荐默认 T1+T2+T3 |
| 07 全景工作台 | **待确认（看板已补 #scene-07）** · 推荐 P-KPI(无左条)+P-CHART+P-FILTER+P-ACTION+P-CREATE |
| 08 档案袋入口 IA | **待确认（#scene-08 入口重画）** · 本人直达 + 名册进入 + 首建启用；顶栏 Select 降级；质量隔离 · `PORTFOLIO_ENTRY_IA_08.md` |
...
- **F-FLOW-QUALITY（已纠偏）**：**否决**「换域 / 学期任务链总壳 / 治理旅程壳」。定案=侧栏**不同菜单→不同主页面**（`quality.ts` 现网）；`QualityScopeChrome` 仅页内分析范围+方案门禁。详见 `QUALITY_ENTRY_IA.md`。
- **F-DOMAIN-FOCUS**：顶层仅域切换；域内只渲染当前旅程。
| 09 本场带队运营（扫描·阅卷·学情） | **待确认（#scene-09 源码合同版）** · 见 `TEAM_LEAD_CONTRACT_09.md`：主考=createUser；扫描共享队列无指派；阅卷=题组+策略+任务池；超管配置出幕 |
...
| 03 考试内旅程+概览 | **待确认（看板已补 #scene-03）** · E-FLOW + E-OVERVIEW（**禁功能叶/更多**） |
